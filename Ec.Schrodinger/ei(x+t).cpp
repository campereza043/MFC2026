...
int main() {
    ...
    cout << "--- Configuración Problema 3 Modificado (u = e^{i(x+t)}) ---" << endl;
    cout << "Delta x (h): " << h << " | Delta t (k): " << k << endl;
    cout << "Pasos totales: " << N_steps << endl;

    auto start_time = chrono::high_resolution_clock::now();

    // 1. Condición Inicial: u(x,0) = e^{ix}
    VectorXcd U(m + 1);
    for (int i = 0; i <= m; ++i) {
        double x = a + i * h;
        U(i) = exp(-x); 
    ...
    // 2. Construcción de Matrices A y B para Crank-Nicolson
    int n_int = m - 1;
    MatrixXcd A = MatrixXcd::Zero(n_int, n_int);
    MatrixXcd B = MatrixXcd::Zero(n_int, n_int);
    
    // Diagonal y subdiagonales para el operador de Schrödinger
    ...
    for (int nn = 0; nn < N_steps; ++nn) {
        ...
        // Fronteras basadas en u(x,t) = exp(i * (x + t))
        cd bc_L_n = exp(eye * (a + t_n));
        cd bc_R_n = exp(eye * (b + t_n));
        cd bc_L_np1 = exp(eye * (a + t_np1));
        cd bc_R_np1 = exp(eye * (b + t_np1));

        // Vector de corrección por fronteras Dirichlet
        ...
        // Actualizar valores en los bordes
        U(0) = bc_L_np1;
        U(m) = bc_R_np1;

        // Exportar datos de este paso de tiempo
        for (int i = 0; i <= m; ++i) {
