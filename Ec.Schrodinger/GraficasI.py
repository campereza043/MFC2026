...
for i, t in enumerate(tiempos_a_graficar):
    ...
    mask = data[:, 0] == t
    x = data[mask, 1]
    u_imag = data[mask, 3]  # Extraemos la parte imaginaria para graficar en la misma gráfica
    
    # Graficamos con puntos ('o') y una línea muy tenue de fondo
    plt.plot(x, u_imag, '*', color=colores_estandar[i % len(colores_estandar)], 
...
# 4. Estética
plt.title('Evolución de la Parte Imaginaria (Puntos de la Malla C++)', fontsize=14)
plt.xlabel('Posición (x)', fontsize=12)
plt.ylabel('Im(ψ)', fontsize=12)
...
# Guardar y mostrar
plt.savefig('TiemposI.png', dpi=300)
plt.show()