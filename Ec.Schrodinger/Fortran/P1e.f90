program schrodinger_cn
    ...
    complex(8), dimension(m-1) :: rhs, sol
    ! Variable para el Error Global Único
    real(8) :: error_global_max, err_local
    ...
    lam = k / (h**2)
    n_int = m - 1
    error_global_max = 0.0d0  ! Inicializamos en cero

    ! Condición Inicial
    do ii = 0, m
        x_full(ii) = a + ii * h
        U_full(0, ii) = phi(x_full(ii))
        ! Opcional: checar error en t=0 (debería ser casi 0)
        err_local = abs(U_full(0, ii) - u_exacta(x_full(ii), 0.0d0))
        if (err_local > error_global_max) error_global_max = err_local
    end do
    ...
    do nn = 0, N_steps - 1
        ...
        ! Resolver sistema (Thomas)
        call thomas_complex(-lam, 2.0d0*eye + 2.0d0*lam, -lam, rhs, sol, n_int)
        ...
        ! COMPARACIÓN PASO A PASO PARA EL ERROR GLOBAL MÁXIMO
        do ii = 0, m
            err_local = abs(U_full(nn+1, ii) - u_exacta(x_full(ii), t_np1))
            if (err_local > error_global_max) error_global_max = err_local
        end do
    end do
    ...
    ! --- SALIDA DE DATOS ---
    print *, "==============================================="
    print '(A, E15.8)', " ERROR GLOBAL MAXIMO (L-inf): ", error_global_max
    print '(A, F10.5, A)', " Tiempo de ejecucion:        ", (end_time - start_time), " s"
    print *, "==============================================="
...
contains
    ...
    subroutine thomas_complex(a_v, b_v, c_v, b_vec, x_vec, n_size)
        integer, intent(in) :: n_size
        complex(8), intent(in) :: a_v, b_v, c_v
        complex(8), dimension(n_size), intent(in) :: b_vec
        ...
        dp(1) = b_vec(1) / b_v
        do i = 2, n_size
            cp(i) = c_v / (b_v - a_v*cp(i-1))
            dp(i) = (b_vec(i) - a_v*dp(i-1)) / (b_v - a_v*cp(i-1))
        end do
    ...
    subroutine export_dat(U, x, steps, space_m, dt)
        ...
        do t_idx = 0, steps
            do x_idx = 0, space_m
                write(20, '(5F13.6)') t_idx*dt, x(x_idx), real(U(t_idx, x_idx)), &
