program schrodinger_cn
    ...
    real(8), parameter :: k = 0.0011d0
    ! --- CAMBIO DE DOMINIO ---
    real(8), parameter :: pi = 3.141592653589793d0
    real(8), parameter :: a = -pi
    real(8), parameter :: b = pi
    ! -------------------------
    integer, parameter :: N_steps = 909 
    ...
    ! EXPORTAR A .DAT
    print *, "Guardando resultados en resultados_2.dat..."
    call export_dat(U_full, x_full, N_steps, m, k)
    print *, "Archivo .dat generado con exito!"

contains

    complex(8) function phi(x)
        real(8), intent(in) :: x
        phi = cmplx(cos(x), 0.0d0, 8)
    end function

    complex(8) function bc_left(t)
        real(8), intent(in) :: t
        bc_left = exp(eye * t) * cos(-pi)
    end function

    complex(8) function bc_right(t)
        real(8), intent(in) :: t
        bc_right = exp(eye * t) * cos(pi)
    end function

    complex(8) function u_exacta(x, t)
        real(8), intent(in) :: x, t
        u_exacta = exp(eye * t) * cos(x)
    end function
    ...
    subroutine export_dat(U, x, steps, space_m, dt)
        ...
        open(unit=20, file='resultados_2.dat', status='replace')
