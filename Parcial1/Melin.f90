PROGRAM MELIN
  ...
  ! Parametros
  INTEGER, PARAMETER :: N = 10, NPT = 600
  REAL(8), PARAMETER :: H = 0.050D0, ALFA = 0.2D0
...
CONTAINS
  ...
  SUBROUTINE DERIVS(X, Y, DYDT, ALFA, DX, N)
    ...
    ! Frontera aislada (derivada = 0 en x=l)
  ...
  SUBROUTINE RK4(Y, DYDT, N, X, H, YOUT, ALFA, DX)
    IMPLICIT NONE
    INTEGER, INTENT(IN)  :: N
    REAL(8), INTENT(IN)  :: Y(N), DYDT(N), X, H, ALFA, DX
  ...
  SUBROUTINE SOL(T, Y, DYDT, H, N, NPT, ALFA, DX)
    ...
    ! Primer paso - escribir condicion inicial
    FR_DER = (4.0D0*Y(N) - Y(N-1)) / 3.0D0
